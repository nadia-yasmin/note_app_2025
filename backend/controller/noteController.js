...
class noteController {
  async addNote(req, res) {
    try {
      ...
      if (!errors.isEmpty()) {
        return res
          .status(400)
          .json(failure("Validation failed", errors.array()));
      }
    ...
    try {
      ...
      if (prev_note.email !== email && user.role !== "admin") {
        return res
          ...
          .send(
            failure(
              "Updating is not possible. Only the note owner or an admin can delete it."
            )
          );
      }
 ...
 const allNotes = await noteModel.find({ email: email });
      ...
      const io = req.app.get("socketio");
         if (io) {
      console.log("Emitting updated notes to all clients...", allNotes);
      io.emit("noteUpdated", allNotes); 
    } else {
  ...
  async showAllNotes(req, res) {
    try {
      ...
      return res
        .status(200)
        .send(success("Notes retrieved successfully", allNotes));
    } catch (error) {
    ...
    try {
      ...
      if (note.email !== email && user.role !== "admin") {
        return res
          ...
          .send(
            failure(
              "Deleting is not possible. Only the note owner or an admin can delete it."
            )
          );
      }
      ...
      return res
        .status(200)
        .send(success("Note deleted successfully", deletedNote));
    } catch (error) {
  ...
  async addEditors(req, res) {
    try {
      ...
      if (note.email !== email && user.role !== "admin") {
        return res
          .status(403)
          .send(failure("Only the note owner or an admin can add editors"));
      }
      ...
      if (note.noteId.includes(editor._id)) {
        return res
          .status(400)
          .send(failure("This user is already an editor for this note"));
      }
